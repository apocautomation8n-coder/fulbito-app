import type { AdminClubReview, Booking, Court, OpenMatch } from '../types/domain';

export const featuredCourts: Court[] = [
  {
    id: 'court-1',
    clubName: 'La Docta Fútbol',
    name: 'Cancha 1',
    format: '7v7',
    neighborhood: 'Nueva Córdoba',
    pricePerSlot: 28000,
    durationMinutes: 60,
    distanceKm: 1.8,
    rating: 4.7,
    nextSlotLabel: 'Hoy 21:00',
    emoji: '🏟️',
    surfaceType: 'Sintético',
  },
  {
    id: 'court-2',
    clubName: 'Barrio Norte Club',
    name: 'Sintético A',
    format: '5v5',
    neighborhood: 'Alta Córdoba',
    pricePerSlot: 22000,
    durationMinutes: 60,
    distanceKm: 3.2,
    rating: 4.5,
    nextSlotLabel: 'Mañana 19:00',
    emoji: '⚽',
    surfaceType: 'Sintético',
  },
  {
    id: 'court-3',
    clubName: 'Predio Kempes',
    name: 'Cancha Grande',
    format: '11v11',
    neighborhood: 'Chateau Carreras',
    pricePerSlot: 64000,
    durationMinutes: 90,
    distanceKm: 6.4,
    rating: 4.8,
    nextSlotLabel: 'Sábado 18:00',
    emoji: '🥅',
    surfaceType: 'Césped natural',
  },
  {
    id: 'court-4',
    clubName: 'Complejo El Bosque',
    name: 'Cancha 3',
    format: '5v5',
    neighborhood: 'Güemes',
    pricePerSlot: 20000,
    durationMinutes: 60,
    distanceKm: 2.1,
    rating: 4.3,
    nextSlotLabel: 'Hoy 22:30',
    emoji: '🌿',
    surfaceType: 'Sintético',
  },
  {
    id: 'court-5',
    clubName: 'Centro Deportivo Sur',
    name: 'Cancha Techada',
    format: '6v6',
    neighborhood: 'General Paz',
    pricePerSlot: 30000,
    durationMinutes: 60,
    distanceKm: 4.5,
    rating: 4.6,
    nextSlotLabel: 'Domingo 16:00',
    emoji: '🏆',
    surfaceType: 'Piso de goma',
  },
  {
    id: 'court-6',
    clubName: 'Club Centenario',
    name: 'Cancha B',
    format: '7v7',
    neighborhood: 'Centro',
    pricePerSlot: 25000,
    durationMinutes: 60,
    distanceKm: 0.8,
    rating: 4.4,
    nextSlotLabel: 'Hoy 20:00',
    emoji: '⭐',
    surfaceType: 'Sintético premium',
  },
];

export const upcomingBookings: Booking[] = [
  {
    id: 'booking-1',
    courtName: 'Cancha 1',
    clubName: 'La Docta Futbol',
    startsAtLabel: 'Hoy 21:00',
    amount: 28000,
    status: 'pending_payment',
    paymentMode: 'full',
  },
  {
    id: 'booking-2',
    courtName: 'Sintetico A',
    clubName: 'Barrio Norte Club',
    startsAtLabel: 'Viernes 20:00',
    amount: 11000,
    status: 'paid',
    paymentMode: 'deposit',
  },
];

export const openMatches: OpenMatch[] = [
  {
    id: 'match-1',
    courtName: 'La Docta Fútbol - Cancha 1',
    neighborhood: 'Nueva Córdoba',
    startsAtLabel: 'Hoy 21:00',
    format: '7v7',
    spotsNeeded: 3,
    pricePerPlayer: 2500,
    status: 'open',
    emoji: '🏟️',
    hostName: 'Ezequiel C.',
  },
  {
    id: 'match-2',
    courtName: 'Barrio Norte Club - Sintético A',
    neighborhood: 'Alta Córdoba',
    startsAtLabel: 'Mañana 19:00',
    format: '5v5',
    spotsNeeded: 1,
    pricePerPlayer: 2200,
    status: 'payment_pending',
    emoji: '⚽',
    hostName: 'Mateo G.',
  },
  {
    id: 'match-3',
    courtName: 'Complejo El Bosque - Cancha 3',
    neighborhood: 'Güemes',
    startsAtLabel: 'Viernes 20:00',
    format: '5v5',
    spotsNeeded: 4,
    pricePerPlayer: 2000,
    status: 'open',
    emoji: '🌿',
    hostName: 'Ignacio R.',
  },
  {
    id: 'match-4',
    courtName: 'Centro Deportivo Sur - Cancha Techada',
    neighborhood: 'General Paz',
    startsAtLabel: 'Sábado 18:00',
    format: '6v6',
    spotsNeeded: 0,
    pricePerPlayer: 2800,
    status: 'full',
    emoji: '🏆',
    hostName: 'Francisco M.',
  },
  {
    id: 'match-5',
    courtName: 'Club Centenario - Cancha B',
    neighborhood: 'Centro',
    startsAtLabel: 'Hoy 22:00',
    format: '7v7',
    spotsNeeded: 2,
    pricePerPlayer: 2400,
    status: 'open',
    emoji: '⭐',
    hostName: 'Julián P.',
  },
];

export const clubsPendingReview: AdminClubReview[] = [
  {
    id: 'club-review-1',
    clubName: 'La Docta Futbol',
    neighborhood: 'Nueva Cordoba',
    ownerEmail: 'admin@ladocta.example',
    submittedAtLabel: 'Hoy 10:24',
    courtsCount: 3,
  },
  {
    id: 'club-review-2',
    clubName: 'Barrio Norte Club',
    neighborhood: 'Alta Cordoba',
    ownerEmail: 'dueno@barrionorte.example',
    submittedAtLabel: 'Ayer 18:40',
    courtsCount: 2,
  },
];
